...
AI coding tools are powerful, but managing multiple agents across different windows is chaos. Megon puts everything in one place: parallel agents, live terminal output, permission gates, and diff review. Stay in control while your agents do the work.
...
megon run "refactor the authentication module"
...
Commands:
  megon                    Start the interactive TUI (default)
  megon run [message..]    Run megon with a message
  megon agent              Manage agents
  megon providers          Manage AI providers and credentials
  megon upgrade [target]   Upgrade to the latest or a specific version
  megon models [provider]  List available AI models
  megon mcp                Manage MCP servers
  megon plugin <module>    Install plugin and update config
  megon session            Manage sessions
  megon serve              Start a headless megon server
  megon web                Start server and open web interface
  megon attach <url>       Attach to a running megon server
  megon export [sessionID] Export session data as JSON
  megon import <file>      Import session data from JSON file or URL
  megon github             Manage GitHub agent
  megon pr <number>        Fetch and checkout a GitHub PR branch
  megon stats              Show token usage and cost statistics
  megon uninstall          Uninstall megon and remove all related files
  megon debug              Debugging and troubleshooting tools
  megon db                 Database tools
  megon completion         Generate shell completion script
...
# Run a one-shot task
megon run "add unit tests for the auth module"
