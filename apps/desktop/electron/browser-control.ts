...
const PARTITION = "persist:megon-browser";
...
function degenModeScript(enabled: boolean, messagePrefix: string) {
  return `(() => {
    ...
    const stateKey = "__megonBrowserDegenState";
    ...
    overlay.setAttribute("data-megon-browser-degen-overlay", "true");
    ...
    badge.setAttribute("data-megon-browser-degen-badge", "true");
    ...
    const onMove = (event) => {
      ...
      if (target === overlay || target === badge || target.closest("[data-megon-browser-degen-overlay],[data-megon-browser-degen-badge]")) return;
...
export function createBrowserControl(options: BrowserControlOptions) {
  ...
  const degenMessagePrefix = `__MEGON_BROWSER_DEGEN_PICK__${token}:`;
  ...
  const ensureCursorScript = async () => {
    ...
    if (!cursorOverlay) {
      await executeInPage(
        `(() => { try {
          const el = document.getElementById("__megon-agent-cursor__");
          if (el) el.remove();
          if (window.__megonCursor) delete window.__megonCursor;
          return true;
    ...
    await executeInPage(
      `(() => {
        try {
          if (window.__megonCursor && document.getElementById("__megon-agent-cursor__")) return true;
          const ID = "__megon-agent-cursor__";
  ...
  const emitCursor = (x: number, y: number, kind: "move" | "down" | "up" | "click", label?: string) => {
    ...
    void executeInPage(
      `(() => { try {
        if (!window.__megonCursor) return false;
        window.__megonCursor.moveTo(${xJs}, ${yJs}, ${kJs}, ${lJs});
        return true;
      } catch (e) { return false; } })()`,
      false,
    );
  ...
  const attachLifecycle = (contents: WebContents) => {
      ...
      if (viewport.mobile && viewport.width > 0) {
        ...
        void executeInPage(
          `(() => { try {
            ...
            let meta = document.querySelector('meta[name="viewport"][data-megon-emulated="1"]');
            if (!meta) {
              meta = document.createElement("meta");
              meta.setAttribute("name", "viewport");
              meta.setAttribute("data-megon-emulated", "1");
    ...
    contents.on("console-message", (_event, _level, message) => {
      ...
      try {
        ...
        console.warn("[megon] browser degen selection failed:", error);
      }
    });
      ...
      if (isMainFrame) {
        ...
        if (validatedURL && !validatedURL.startsWith("data:text/html")) {
          void contents.loadURL(errorPageDataUrl(validatedURL, lastNavigationError, browserTheme)).catch((error) => {
            console.warn("[megon] browser error page failed:", error);
          });
        }
      }
      publish();
    ...
    contents.setWindowOpenHandler(({ url }) => {
      void contents.loadURL(url).catch((error) => console.warn("[megon] browser popup navigation failed:", error));
      return { action: "deny" };
    });
  };
  ...
  const addToWindow = () => {
    ...
    if (!visible) {
      try {
        ...
        console.warn("[megon] browser attach failed:", error);
      }
      visible = true;
    }
  ...
  const safeHide = async () => {
    await hide().catch((error) => {
      console.warn("[megon] browser hide state failed:", error);
    });
  };
  ...
  const executeInPage = async <T>(source: string, fallback: T): Promise<T> => {
    ...
    try {
      ...
      console.warn("[megon] browser page script failed:", error);
      return fallback;
    }
  };
    ...
    executeInPage<BrowserElementSnapshot[]>(
      `(() => {
        ...
        return elements.flatMap((el) => {
          ...
          if (hidden) return [];
          const htmlEl = el;
          let ref = htmlEl.getAttribute("data-megon-browser-ref");
          if (!ref) {
            ref = "e" + (++index).toString(36);
            htmlEl.setAttribute("data-megon-browser-ref", ref);
          }
  ...
  const focusRef = async (ref: string | undefined) => {
    if (!ref) return;
    const selector = JSON.stringify(`[data-megon-browser-ref="${ref.replace(/"/g, '\\"')}"]`);
    await executeInPage(
      `(() => {
    ...
    switch (action) {
      ...
      case "set_viewport": {
        ...
        try {
          ...
          console.warn("[megon] browser setUserAgent failed:", error);
        }
      ...
      req.on("end", () => {
        void (async () => {
          try {
            ...
            if (req.headers["x-megon-browser-token"] !== token) {
