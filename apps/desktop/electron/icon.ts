...
export const WINDOWS_APP_ID = app.isPackaged ? "app.megon.desktop" : "app.megon.desktop.dev";
...
export function resolveAppIconPath(platform: NodeJS.Platform = process.platform) {
  ...
  if (!app.isPackaged) {
    candidates.push(
      ...iconCandidates(path.join(__dirname, "..", "electron", "dev-icons"), "megon-dev", fallbackExts),
      ...iconCandidates(path.join(__dirname, "..", "src", "assets", "icon"), "megon-dev", fallbackExts),
    );
  }
