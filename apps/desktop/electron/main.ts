...
const RELEASES_URL = "https://github.com/MegonLabs/megon/releases/latest";
...
function sendAppEvent(channel: string, payload: any) {
  ...
  mainWindow.webContents.send("megon:event", {
    channel,
    payload,
  });
}
...
async function checkForUpdatesWithRetry() {
  try {
    ...
    console.warn("[megon] transient update check failure, retrying:", error);
...
function setupAutoUpdater() {
  ...
  autoUpdater.on("checking-for-update", () => {
    console.log("[megon] checking for update...");
    sendUpdateEvent("checking", null);
  });

  autoUpdater.on("error", (error) => {
    console.warn("[megon] auto update failed:", error);
    updateDownloadInFlight = false;
    sendUpdateEvent("error", formatUpdateError(error));
  });

  autoUpdater.on("update-available", (info) => {
    console.log("[megon] update available:", info.version);
    ...
    autoUpdater.downloadUpdate().catch((error) => {
      updateDownloadInFlight = false;
      console.warn("[megon] failed to auto-download update:", error);
      sendUpdateEvent("error", formatUpdateError(error));
    });
  });

  autoUpdater.on("update-not-available", (info) => {
    console.log("[megon] update not available:", info.version);
  ...
  autoUpdater.on("download-progress", (progressObj) => {
    console.log("[megon] download progress:", progressObj.percent);
  ...
  autoUpdater.on("update-downloaded", (info) => {
    console.log("[megon] update downloaded:", info.version);
    availableUpdateVersion = info.version;
...
function queuePtyOutput(id: string, data: string) {
  ...
  setTimeout(() => {
    ...
    mainWindow.webContents.send("megon:terminal-data", { id, data: payload });
  }, 16);
}
...
function finishPty(id: string, session: PtyRuntime) {
  ...
  mainWindow?.webContents.send("megon:terminal-exit", { id });
}
...
const MEGON_SKILL_STORE_MARKER = "<!-- megon-skill-store";
const MEGON_SKILL_STORE_MARKER_FILE = ".megon-skill-store.json";
const MEGON_SKILL_STORE_SOURCE = "megon/builtin-skills";
const LEGACY_DUMMY_SKILL_TEXT = "This Megon store skill provides workflow guidance only.";
...
function normalizeSkillDescription(value?: string | null) {
  return value?.replace(/\s+/g, " ").replace(/\.$/, "").trim() || "Built-in Megon skill";
}
...
async function readManagedMarker(skillId: string) {
  const { target } = resolveManagedSkillDir(skillId);
  try {
    return await fs.readFile(path.join(target, MEGON_SKILL_STORE_MARKER_FILE), "utf8");
  } catch (error: any) {
    if (error?.code === "ENOENT") return null;
    throw error;
  }
...
function isLegacyDummySkill(skillId: string, content: string | null) {
  return Boolean(content?.includes(`${MEGON_SKILL_STORE_MARKER}:${skillId}`) && content.includes(LEGACY_DUMMY_SKILL_TEXT));
}
...
async function readSkillInstallState(skillId: string) {
  ...
  return {
    ...
    managed: markerData?.source === MEGON_SKILL_STORE_SOURCE || legacyDummy,
    deprecatedManaged: Boolean(markerData?.source && markerData.source !== MEGON_SKILL_STORE_SOURCE),
    legacyDummy,
  };
}
...
async function installSkill(skillId: string) {
  ...
  try {
    ...
    await fs.writeFile(
      path.join(target, MEGON_SKILL_STORE_MARKER_FILE),
      JSON.stringify(
        {
          source: MEGON_SKILL_STORE_SOURCE,
          skillId: item.id,
...
async function uninstallSkill(skillId: string) {
  ...
  if (!state.managed) {
    throw new Error("This skill was not installed by Megon Skill Store.");
  }
...
const handlers: Record<string, (payload?: any) => Promise<any> | any> = {
    ...
    try {
      ...
      if (manual && result && !result.isUpdateAvailable) {
        await dialog.showMessageBox({
          type: "info",
          title: "Up to Date",
          message: "You are already running the latest version of Megon.",
        });
      }
      return {
        status: "success",
      ...
      console.warn("[megon] update check failed:", error);
  ...
  download_update: async () => {
    ...
    try {
      ...
      updateDownloadInFlight = false;
      console.warn("[megon] failed to start update download:", error);
      return { status: "error", message: error instanceof Error ? error.message : String(error) };
    }
  },
  MEGON_server_start: async () => {
...
function registerIpc() {
  ipcMain.handle("megon:invoke", async (_event, command, payload = {}) => {
    const handler = handlers[command];
    if (!handler) throw new Error(`Unknown IPC command: ${command}`);
    return handler(payload);
  });

  ipcMain.on("megon:storage-get", (event, storage, key) => {
    try {
      event.returnValue = storageGet(storage, key);
    } catch (error) {
      console.warn("[megon] failed to read storage:", error);
      event.returnValue = null;
    }
  });

  ipcMain.on("megon:get-megon-server-url", (event) => {
    event.returnValue = serverInstance?.url ?? null;
  });

  ipcMain.handle("megon:terminal-spawn", async (_event, options) => {
  ...
  ipcMain.handle("megon:terminal-write", (_event, id, data) => {
    const session = ptySessions.get(id);
    if (session?.status === "running") session.proc.write(data);
  });

  ipcMain.handle("megon:terminal-resize", (_event, id, cols, rows) => {
    const session = ptySessions.get(id);
  ...
  ipcMain.handle("megon:terminal-kill", (_event, id) => {
    killPty(id);
  });
}
...
async function createWindow() {
  ...
  mainWindow.on("maximize", () => mainWindow?.webContents.send("megon:window-state", { maximized: true }));
  mainWindow.on("unmaximize", () => mainWindow?.webContents.send("megon:window-state", { maximized: false }));
  mainWindow.on("enter-full-screen", () =>
    mainWindow?.webContents.send("megon:window-state", { fullscreen: true }),
  );
  mainWindow.on("leave-full-screen", () =>
    mainWindow?.webContents.send("megon:window-state", { fullscreen: false }),
  );
  ...
  const hideEmbeddedBrowserForHostReload = () => {
    void browserControl?.hide().catch((error) => {
      console.warn("[megon] failed to hide embedded browser during host reload:", error);
    });
  };
...
app.whenReady().then(async () => {
  ...
  process.env.MEGON_BROWSER_CONTROL_URL = browserEndpoint.url;
  process.env.MEGON_BROWSER_CONTROL_TOKEN = browserEndpoint.token;
  registerIpc();
  // Start server asynchronously in the background so the window opens instantly
  void ensureServerStarted().catch((err) => {
    console.error("[megon] failed to start server in background:", err);
  });
  await createWindow();
  setupAutoUpdater();
  app.on("activate", () => {
