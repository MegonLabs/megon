...
const SIDECAR_CRASH_FILE = path.join(os.tmpdir(), "megon-sidecar-crash.log")
...
const SIDECAR_SERVICE_NAME = "megon-server"
...
function platformDataEnv(): Record<string, string> {
  ...
  // The server appends its app namespace ("Megon") onto each XDG base, so point
  // the bases at the macOS parent dirs — they resolve to ~/Library/Application
  // Support/megon (data/config/state) and ~/Library/Caches/megon (cache).
...
async function startPackagedServer(
  ...
  console.log(`[megon] starting packaged server on ${hostname}:${port}`)
  console.log(`[megon] sidecar: ${sidecarScript}`)
  console.log(`[megon] server bundle: ${serverBundlePath}`)
  ...
  const child = utilityProcess.fork(sidecarScript, [], {
    ...
    env: {
      ...
      ...platformDataEnv(),
      NODE_PATH: nodePath,
      MEGON_DISABLE_EMBEDDED_WEB_UI: "true",
    },
  ...
  const url = await new Promise<string>((resolve, reject) => {
    ...
    child.once("exit", (code) => {
      exited = true
      exitCode = code
      console.warn(`[megon] sidecar exited with code ${code}`)
      if (!resolved) {
        resolved = true
    ...
    child.on("error", (err: unknown) => {
      console.error("[megon] sidecar error:", err)
      if (!resolved) {
        resolved = true
        clearTimeout(timeout)
  ...
  console.log(`[megon] server listening at ${url}`)

  try {
    await waitForHealth(url)
    console.log(`[megon] server health check passed`)
  } catch (err) {
    if (!exited) child.kill()
    throw err
  }
...
async function startDevServer(
  ...
  console.log(`[megon] starting server on ${hostname}:${port} with ${bunPath}`)
  ...
  const url = await new Promise<string>((resolve, reject) => {
    ...
    proc.stdout?.on("data", (chunk: Buffer) => {
      ...
      const match = output.match(/megon server listening on (https?:\/\/[^\s]+)/i)
      if (match && match[1]) {
        resolved = true
        clearTimeout(timeout)
        resolve(match[1])
