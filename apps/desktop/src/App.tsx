...
import { Toast, showToast } from '@megon/ui/toast'
      ...
      try {
        unlistenAvailable = await nativeApi.listen<{ version: string }>("update:available", (event) => {
          if (windowChrome.isMac()) {
            ...
            showToast({
              ...
              description: `Megon ${event.payload.version} is available. Open the Releases page to download it.`,
              variant: "default",
              persistent: true,
          ...
          showToast({
            ...
            description: `Megon ${event.payload.version} is downloading in the background. You can keep working.`,
            variant: "default",
            duration: 8000,
            leading: updateToastLogo(),
            actions: [
        ...
        unlistenDownloaded = await nativeApi.listen<{ version: string }>("update:downloaded", (event) => {
          showToast({
            ...
            description: `Megon ${event.payload.version} has finished downloading. Restart now to complete the update.`,
            variant: "success",
            persistent: true,
            leading: updateToastLogo(),
            actions: [
