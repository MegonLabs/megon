...
import { PromptInput } from "../megon-ported/prompt-input"
import { sendFollowupDraft, type FollowupDraft } from "@/megon-ported/prompt-input/submit"
import { MockSessionProviders } from "../megon-ported/mock-session-layout"
import { useSync } from "@/context/sync"
import { useGlobalSync } from "@/context/global-sync"
import { useNavigate, useParams } from "@solidjs/router"
import { AssistantPartGroupView, Message } from "@megon/ui/message-part"
import { FileIcon } from "@megon/ui/file-icon"
import { DataProvider, FileComponentProvider } from "@megon/ui/context"
import { AppIcon } from "@megon/ui/app-icon"
import type { IconName } from "@megon/ui/icons/app"
import { Icon } from "@megon/ui/icon"
import { sessionTitle } from "@/utils/session-title"
import { MacSidebarReveal } from "./mac-chrome"
import { useWindowChrome } from "@/utils/window-chrome"
import { createSessionComposerState } from "@/megon-ported/composer/session-composer-state"
import { SessionQuestionDock } from "@/megon-ported/composer/session-question-dock"
import { SessionPermissionDock } from "@/megon-ported/composer/session-permission-dock"
import { SessionTodoDock } from "@/megon-ported/composer/session-todo-dock"
import { useLanguage } from "@/context/language"
import { File as MegonFile } from "@megon/ui/file"
import { createAutoScroll } from "@megon/ui/hooks"
import { createChatFind } from "@megon/ui/pierre/file-find"
import { FileSearchBar } from "@megon/ui/file-search"
import { createResizeObserver } from "@solid-primitives/resize-observer"
import type { EventSessionError, Message as ChatMessage, Part, SessionStatus } from "@megon/sdk/v2/client"
import { TextField } from "@megon/ui/text-field"
import { Button } from "@megon/ui/button"
import { formatError } from "@/pages/error"
import { useStore } from "../store"
import { nativeApi } from "@/services/native"
import { useSettings } from "@/context/settings"
import { TextShimmer } from "@megon/ui/text-shimmer"
import { Card, CardDescription, CardTitle } from "@megon/ui/card"
import { SessionRetry } from "@megon/ui/session-retry"
import { showToast } from "@megon/ui/toast"
import { useDialog } from "@megon/ui/context/dialog"
...
import { Dialog } from "@megon/ui/dialog"
...
function AgentViewInner(props: AgentViewProps) {
  ...
  const syncMegonSessions = useStore((s) => s.syncMegonSessions)
  const language = useLanguage()
  ...
  onMount(() => {
    setTitlebarLeftEl(document.getElementById("megon-titlebar-left"))
    setTitlebarRightEl(document.getElementById("megon-titlebar-right"))
  })
  onMount(() => {
  ...
  const FilePreview = (fileProps: any) => <MegonFile {...fileProps} />

  const dialog = useDialog()
  const promptCtx = usePrompt()

  const handleRevert = async (userMessageID: string, action: "edit" | "resubmit") => {
    const sessionID = activeSessionId()
    if (!sessionID) return
    const message = messageByID().get(userMessageID)
    if (!message) return

    const parts = getParts(userMessageID)
    const textParts = (parts as any[]).filter((p) => p.type === "text")
    const promptText = textParts.map((p) => p.text).join("")

    // Use per-message diffs from the user message's summary (changes from this specific response only)
    const summaryDiffs = (message as any).summary?.diffs
    const files = (Array.isArray(summaryDiffs) ? summaryDiffs : []) as Array<{ file?: string; additions: number; deletions: number }>

    dialog.show(() => {
      const [confirming, setConfirming] = createSignal(false)
      const isRegenerate = action === "resubmit"
      return (
        <Dialog title={isRegenerate ? "Confirm Regenerate" : "Confirm Undo"} fit compact>
          <div class="flex flex-col">
            <p class="text-[13px] leading-[1.4] text-text-weak px-5 pt-0.5 pb-3">
              {isRegenerate
                ? "Confirming this regenerate action will undo the changes and regenerate the response."
                : files.length > 0
                  ? "Confirming this undo action will make the following changes:"
                  : "This undo action will not make any code changes."}
            </p>
            <Show when={files.length > 0}>
              <div class="max-h-[180px] overflow-y-auto rounded-lg border border-border-weak-base bg-surface-base p-1 mx-5 mb-3">
                <For each={files}>
                  {(file) => (
                    <div class="flex items-center gap-2 px-2 py-1 text-[13px] text-text-strong rounded-md hover:bg-surface-base-hover transition-colors">
                      <FileIcon node={{ path: file.file ?? "unknown", type: "file" }} class="shrink-0 size-[16px]" />
                      <span class="truncate flex-1 min-w-0">{file.file ?? "unknown"}</span>
                      <span class="shrink-0 text-[11px] leading-none tabular-nums font-medium whitespace-nowrap">
                        <Show when={(file.additions || 0) > 0}>
                          <span class="text-green-500">+{file.additions}</span>
                        </Show>
                        <Show when={(file.additions || 0) > 0 && (file.deletions || 0) > 0}>
                          {" "}
                        </Show>
            ...
            <div class="flex items-center justify-end gap-2 px-5 py-3 border-t border-border-weak-base">
              <button
                ...
                class="px-3.5 py-1.5 text-[13px] font-medium text-text-strong rounded-lg hover:bg-surface-base-hover transition-colors cursor-pointer"
              >
                Cancel
              </button>
              <button
                ...
                onClick={async () => {
                  setConfirming(true)
                  try {
                    await sdk.client.session.revert({ sessionID, messageID: userMessageID })

                    // Clean up reverted messages from the database
                    await fetch(`${sdk.url}/session/${sessionID}/cleanup`, { method: "POST" })

                    await sync.session.sync(sessionID, { force: true })

                    if (action === "edit" && promptText) {
                      promptCtx.set([{ type: "text", content: promptText, start: 0, end: promptText.length }], promptText.length)
                    }

                    showToast({ variant: "success", title: isRegenerate ? "Regenerated" : "Reverted", description: isRegenerate ? "Response regenerated." : "Changes reverted. Prompt restored for editing." })
                    dialog.close()

                    if (action === "resubmit") {
                      requestAnimationFrame(() => {
                        const el = document.querySelector('.agent-terminal-prompt-shell')
                        if (el) {
                    ...
                    showToast({ variant: "error", title: isRegenerate ? "Regenerate failed" : "Revert failed", description: formatServerError(error, language.t, isRegenerate ? "Could not regenerate." : "Could not revert.") })
                  }
                }}
                class="px-3.5 py-1.5 text-[13px] font-medium text-white bg-surface-brand-base hover:bg-surface-brand-hover rounded-lg transition-colors cursor-pointer disabled:opacity-50"
              >
                <Show when={confirming()} fallback="Confirm">
                  {isRegenerate ? "Regenerating..." : "Reverting..."}
                </Show>
              </button>
            </div>
          </div>
                ...
                <div data-component="session-turn" class="relative min-w-0 w-full">
                  <div data-slot="session-turn-message-container" class="w-full">
                    <div data-slot="session-turn-message-content" aria-live="off">
                      <Message
                        message={message()}
                        parts={getParts(row.userMessageID)}
                        actions={{
                          ...(row.userMessageID === latestUserMessageID() && !working()
                            ? {
                                undoChanges: () => handleRevert(row.userMessageID, "edit"),
                                regenerate: () => handleRevert(row.userMessageID, "resubmit"),
                              }
                            : {}),
                        }}
                      />
  ...
  const syncSessionIntoProject = async (targetSessionID: string) => {
    ...
    await syncMegonSessions(project.id, [...sync.data.session, ...existingSessions] as any)
    return true
  }
  ...
  return (
    ...
    >
        ...
        >
          <div class="agent-terminal-scroll-frame relative min-h-0 flex-1 overflow-hidden">
            ...
            <Show when={!windowChrome.isMac() && !useTitlebarPanelControls()}>
              <div class="megon-agent-corner-controls pointer-events-none absolute right-3 top-2 z-40 flex items-center justify-end">
                <div class="megon-agent-corner-toolbar pointer-events-auto">
            ...
            <Show when={!windowChrome.isMac() && useTitlebarPanelControls() && titlebarRightEl()}>
              {(el) => (
                <Portal mount={el()}>
                  <div class="megon-titlebar-panel-controls flex min-w-0 items-center">
                    <AgentHeaderPanelControls projectPath={activeProjectPath()} />
                  </div>
                </Portal>
              )}
            ...
            <div
              ...
              class="agent-terminal-scroll agent-smart-scrollbar relative h-full min-w-0 overflow-x-hidden overflow-y-auto"
              tabIndex={0}
              style={{
              ...
              onPointerDown={(e) => {
                ...
                chatFind.onPointerDown()
              }}
              onFocus={chatFind.onFocus}
              onKeyDown={handleTimelineKeyDown}
            >
              <Show when={chatFind.open()}>
                <div class="sticky top-0 z-40 flex items-center gap-2 bg-background-base/90 backdrop-blur-sm px-3 py-2">
                  <FileSearchBar
                    pos={chatFind.pos}
                    query={chatFind.query}
                    count={chatFind.count}
                    index={chatFind.index}
                    setInput={chatFind.setInput}
                    onInput={chatFind.setQuery}
                    onKeyDown={chatFind.onInputKeyDown}
                    onClose={chatFind.close}
                    onPrev={() => chatFind.next(-1)}
                    onNext={() => chatFind.next(1)}
                    variant="sticky"
                  />
                </div>
              </Show>
              <div class="agent-terminal-scroll-body" onClick={autoScroll.handleInteraction}>
                ...
                <Show when={!windowChrome.isMac() && titlebarLeftEl()}>
                  {(el) => (
                    <Portal mount={el()}>
                      <div class="megon-titlebar-agent-controls flex min-w-0 items-center gap-2">
                        {renderTitleCluster()}
                        {renderBranchSwitcher()}
                      </div>
                    </Portal>
      ...
      <Show when={showDockedComposer()}>
        ...
        >
          <Show when={showJump()}>
            <div class="pointer-events-none absolute bottom-full left-1/2 z-[60] mb-2 -translate-x-1/2">
              <button
                type="button"
