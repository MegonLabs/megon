...
import type { SessionStatus, VcsFileDiff } from '@megon/sdk/v2'
import { SessionReviewTab } from '@/pages/session/review-tab'
import { FileComponentProvider } from '@megon/ui/context'
import { File as MegonFile } from '@megon/ui/file'
import { ScrollView } from '@megon/ui/scroll-view'
...
const BROWSER_TAB_STATE_KEY = "megon.browser-tab-state.v1"
          ...
          <Show when={content()} fallback={<div class="px-6 py-4 text-12-regular text-text-weak">Loading...</div>}>
            {(file) => (
              <div class="relative overflow-hidden pb-40">
                <MegonFile mode="text" file={file()} class="select-text" />
              </div>
            )}
          </Show>
        </ScrollView>
...
function MegonReviewContent(props: {
  projectPath: () => string
  ...
  return (
    <SessionSidePanel
      ...
      allFilesPanel={props.allFilesPanel}
      reviewPanel={() => (
...
export function MainView() {
  ...
  const [activeTabId, setActiveTabId] = createSignal<string>("all-files")
    ...
    on(
      ...
      (path) => {
        ...
        if (!path) {
          ...
          if (activeTabId() === "browser") setActiveTabId("all-files")
  ...
  const openBrowserTab = () => {
    if (!currentProject()) {
      console.warn("[megon-open-browser-tab] no current project")
      return
    }
  ...
  createEffect(() => {
    const handleOpenTerminalTab = async (e: Event) => {
      try {
        ...
        const project = currentProject()
        if (!project) {
          console.warn("[megon-open-terminal-tab] no current project")
          return
        }
        const projectId = currentProjectId() ?? projects()[0]?.id ?? null
        if (!projectId) {
          console.warn("[megon-open-terminal-tab] no project id")
          return
        }
        const shell =
        ...
        console.error("[megon-open-terminal-tab] failed:", err)
      }
    }

    window.addEventListener('megon-open-terminal-tab', handleOpenTerminalTab as EventListener)
    return () => window.removeEventListener('megon-open-terminal-tab', handleOpenTerminalTab as EventListener)
  })
  ...
  createEffect(() => {
    ...
    window.addEventListener('megon-open-browser-tab', handleOpenBrowserTab as EventListener)
    const unlistenPromise = nativeApi.listen("browser:open", handleOpenBrowserTab)
    return () => {
      window.removeEventListener('megon-open-browser-tab', handleOpenBrowserTab as EventListener)
      unlistenPromise.then((unlisten) => unlisten()).catch(() => undefined)
    }
  })
  ...
  const handleCloseTerminalTab = (id: string) => {
    setTerminalTabs((tabs) => {
      ...
      if (activeTabId() === `terminal:${id}`) {
        setActiveTabId(next.at(-1)?.id ? `terminal:${next.at(-1)!.id}` : "all-files")
      }
      return next
    })
  }
  ...
  const handleCreateSession = async () => {
    ...
    const reusable = findReusableEmptyRootMegonSession(projectStore.session, projectStore.message, appStore.activeSessionId)
    if (reusable) {
      ...
      await appStore.syncMegonSessions(cpid, projectStore.session)
    ...
    await appStore.syncMegonSessions(cpid, mergedSessions)
    if (currentProjectId() !== cpid) appStore.setCurrentProject(cpid)
        ...
        {activePage() === 'workspace' ? (
          <LayoutProvider>
            <WorkspaceContent currentProject={currentProject}>
              <div ref={workspaceSplitRef} class="megon-workspace-split flex h-full w-full min-h-0 min-w-0">
                <div
                  class="megon-workspace-agent min-h-0 min-w-0 overflow-hidden"
                  classList={{
                ...
                <Show when={sidePanelMounted()}>
                  <div
                    class="megon-workspace-side-panel relative h-full min-h-0 max-h-full shrink-0 overflow-hidden"
                    classList={{
                      "flex-1": sidePanelOpen() && sidePanelMainOpen(),
                          ...
                          <fileCtx.FileProvider>
                            <MegonReviewContent
                              ...
                              allFilesPanel={() => {
                                ...
                                const kindsMap = new Map<string, "add" | "del" | "mix">()
                                const normalize = (p: string) => p.replaceAll("\\", "/").replace(/\/+$/, "")
                                const merge = (a: string | undefined, b: "add" | "del" | "mix") => {
                                  if (!a) return b
                                  if (a === b) return a
                                  return "mix" as const
                                }
                                for (const diff of diffs) {
                                  const fp = normalize(diff.file)
                                  const kind = diff.status === "added" ? "add" : diff.status === "deleted" ? "del" : "mix"
                                  kindsMap.set(fp, kind)
                                  const parts = fp.split("/")
                                  for (const [idx] of parts.slice(0, -1).entries()) {
                                    const dir = parts.slice(0, idx + 1).join("/")
                                    if (dir) kindsMap.set(dir, merge(kindsMap.get(dir), kind))
                                  }
                                }
                                const modifiedFiles = [...kindsMap.keys()]
                                return (
                                  <div class="flex h-full min-h-0 flex-col overflow-hidden bg-background-stronger">
                                    <div class="flex-1 min-h-0 overflow-y-auto">
                                      <FileTree
                                        ...
                                        kinds={kindsMap}
                                        onFileClick={(node) => handleFileSelect(node.path)}
                                      />
                                    </div>
                                  </div>
                                )
