...
import type { Session as MegonSession, SessionStatus } from "@megon/sdk/v2/client"
...
import { findReusableEmptyRootMegonSession, sortMegonSessionsById } from "@/utils/megon-session"
...
const SidebarSectionTitle = (props: { children: string }) => (
  <div class="megon-sidebar-section-label px-3 pb-2 pt-4 text-[13px] font-normal leading-4 text-text-weaker">
    {props.children}
  </div>
)
...
const SidebarSectionHeader = (props: {
  ...
  return (
    <div class="flex items-center justify-between gap-2 px-3 pb-2 pt-4">
      <div class="megon-sidebar-section-label min-w-0 truncate text-[13px] font-normal leading-4 text-text-weaker">
        {props.children}
      </div>
...
const SidebarActionButton = (props: {
  ...
  return (
    <button
      ...
      class={`group/action megon-sidebar-action grid h-8 w-full min-w-0 grid-cols-[20px_minmax(0,1fr)_auto] items-center gap-2 rounded-[5px] border border-transparent px-2.5 text-left text-[13px] leading-4 transition-colors ${
        props.active
    ...
    >
      ...
      <span class="megon-sidebar-main-label min-w-0 truncate font-normal text-current">{props.label}</span>
      <Show when={props.shortcut}>
        <span class="megon-sidebar-shortcut rounded-full bg-surface-raised-base px-1.5 py-0.5 font-mono text-[11px] leading-3 text-text-weaker">
          {props.shortcut}
        </span>
      </Show>
    </button>
...
const PinnedSessionRow = (props: {
  ...
  return (
    ...
    >
      <span class={`megon-sidebar-main-label min-w-0 truncate text-[13px] font-normal leading-4 ${isWorking() ? "megon-session-shimmer" : ""}`}>{props.hit.session.name}</span>
      <span class="flex min-w-7 items-center justify-end">
        <Switch
          fallback={
            <span class="megon-sidebar-meta text-[12px] font-normal leading-5 text-text-weak">
...
function FolderSection(props: {
  ...
  onSyncMegonSessions: (projectId: string, sessions: MegonSession[]) => void
  ...
  const MegonSessions = createMemo(() => projectStore().session)
      ...
      >
        ...
        <div class="min-w-0 flex flex-1 items-center">
          <Show
            ...
            fallback={
              <span
                class={`megon-sidebar-main-label truncate text-[13px] leading-4 ${
  ...
  createEffect(() => {
    const sessions = MegonSessions()
    if (projectStore().status === "loading" && sessions.length === 0) return
    props.onSyncMegonSessions(props.project.id, sessions)
  })
    ...
    >
      ...
      >
        ...
        >
          ...
          <span class={`megon-sidebar-main-label truncate text-[13px] leading-4 transition-colors ${
            props.currentProjectId === props.project.id
        ...
        <div class="min-h-0 overflow-hidden">
          ...
          >
            ...
            <Show when={hiddenRootSessionCount() > 0}>
              <button
                type="button"
                class="megon-sidebar-main-label h-8 rounded-[5px] pl-[34px] pr-3 text-left text-[13px] font-normal text-text-weaker transition-colors hover:bg-surface-raised-base-hover hover:text-text-base"
                onClick={(e) => {
                  e.stopPropagation()
    ...
    <For each={props.projects}>
      {(project) => (
        <FolderSection
          ...
          onSyncMegonSessions={props.onSyncMegonSessions}
...
export function Sidebar(props: {
  ...
  createEffect(() => {
    ...
    document.documentElement.style.setProperty("--megon-sidebar-width", `${width}px`)
  })
  ...
  const handleCreateSession = async (projectId: string) => {
    ...
    const reusable = findReusableEmptyRootMegonSession(projectStore.session, projectStore.message, activeSessionId())
    if (reusable) {
      ...
      await syncMegonSessions(projectId, projectStore.session)
    ...
    setProjectStore("session", (sessions) =>
      hadSession
        ...
        : sortMegonSessionsById([...sessions, created]),
    )
    ...
    await syncMegonSessions(projectId, mergedSessions)
  ...
  const handleSyncMegonSessions = (projectId: string, sessions: MegonSession[]) => {
    ...
    void syncMegonSessions(projectId, filtered)
  }
  ...
  return (
    ...
    >
      <Show when={isSidebarVisible()}>
        ...
        />
        <div class="megon-sidebar relative flex h-full min-h-0 max-h-full flex-col overflow-hidden bg-background-stronger text-text-base select-none">
          <MacSidebarHeader />
          ...
          <div class="megon-sidebar-scrollbar min-h-0 flex-1 overscroll-contain overflow-y-auto">
              ...
              >
                <div class="flex flex-col gap-2">
                  <SortableProjectGroup
                    ...
                    onSyncMegonSessions={handleSyncMegonSessions}
          ...
          <div class="shrink-0 px-1.5 pb-3 pt-1">
            <div class="rounded-lg bg-surface-raised-base/50 p-3">
              <div class="mb-1.5 flex items-center justify-between">
                <span class="text-[12px] font-medium text-text-strong">Enjoying the app?</span>
                <span class="inline-flex h-5 shrink-0 items-center justify-center rounded-full border border-border-base px-2 py-0.5 text-[10px] font-medium text-text-base whitespace-nowrap">Free & Open Source</span>
              </div>
              <p class="mb-2.5 text-[11px] leading-[1.4] text-text-weak">
                This app is completely free and open source. A GitHub star helps support development and future improvements.
              </p>
              <button
                type="button"
                onClick={() => nativeApi.invoke("open_external", { url: "https://github.com/MegonLabs/megon" })}
                class="flex w-full items-center justify-center gap-1.5 rounded-md bg-surface-raised-strong px-3 py-1.5 text-[12px] font-medium text-text-strong transition-colors hover:bg-surface-raised-strong/80 cursor-pointer"
              >
                <Icon name="star" size="small" class="text-icon-interactive-base" />
