...
import { List } from "@megon/ui/list"
import { Tag } from "@megon/ui/tag"
...
import {
  ...
  MEGON_OPENAI_COMPATIBLE_PRESET,
} from "./megon-settings/dialog-openai-compatible"

const CUSTOM_ID = "_custom"
const MEGON_ID = "_megon"
    ...
    <Dialog title={language.t("command.provider.connect")} transition>
      <List
        ...
        items={() => {
          language.locale()
          return [
            { id: MEGON_ID, name: MEGON_OPENAI_COMPATIBLE_PRESET.name },
            ...
            ...providers.all().filter((x) => x.id !== MEGON_OPENAI_COMPATIBLE_PRESET.providerID),
          ]
        }}
        filterKeys={["id", "name"]}
        groupBy={(x) =>
        ...
        sortBy={(a, b) => {
          if (a.id === MEGON_ID) return -1
          if (b.id === MEGON_ID) return 1
        ...
        onSelect={(x) => {
          ...
          if (x.id === MEGON_ID) {
            dialog.show(() => (
              <DialogOpenAICompatible defaults={MEGON_OPENAI_COMPATIBLE_PRESET} iconID="Megon" apiKeyOnly />
            ))
            return
          }
          if (x.id === CUSTOM_ID) {
        ...
        {(i) => (
          <div class="px-1.25 w-full flex items-center gap-x-3">
            ...
            <Show when={i.id === "opencode"}>
              <div class="text-14-regular text-text-weak">{language.t("dialog.provider.megon.tagline")}</div>
            </Show>
