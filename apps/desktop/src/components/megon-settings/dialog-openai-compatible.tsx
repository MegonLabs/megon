...
export const MEGON_OPENAI_COMPATIBLE_PRESET = {
  providerID: "Megon",
  name: "Megon AI Gateway",
  baseURL: "https://workspace.megon.org/v1",
} as const
...
export function DialogOpenAICompatible(props: Props = {}) {
  ...
  const requestModels = async () => {
    ...
    const auth = currentServer.http.password
      ? {
          Authorization: `Basic ${btoa(`${currentServer.http.username ?? "Megon"}:${currentServer.http.password}`)}`,
        }
      : {}
    ...
    if (!response.ok) {
      if (response.status === 404 && !data.error) {
        throw new Error("Local model fetch route was not found. Restart Megon and try again.")
      }
    ...
    >
      <div class="flex flex-col gap-6 px-2.5 pb-3 overflow-y-auto max-h-[60vh]">
        ...
        <form onSubmit={save} class="px-2.5 pb-6 flex flex-col gap-6">
          <Show when={props.iconID === "Megon"}>
            <p class="text-14-regular text-text-base">
              Enter your Megon AI Gateway API key to connect your account and use Megon models in Megon.
            </p>
          </Show>
          ...
          <div class="flex flex-col gap-4">
            ...
            <TextField
              ...
              description={props.iconID === "Megon" ? undefined : language.t("provider.custom.field.apiKey.description")}
              value={form.apiKey}
