...
const hasProviderIcon = (id: string) =>
  id === "antigravity" || id === "Megon" || id === "openclaude" || iconNames.includes(id as (typeof iconNames)[number])
      ...
      <div class="w-full space-y-8">
        ...
        <Section title={language.t("settings.providers.section.popular")}>
          <div class="grid grid-cols-[repeat(auto-fit,minmax(min(100%,300px),1fr))] gap-3.5">
            <div data-component="megon-compatible-section">
              <SimpleTopActionCard
                iconId="Megon"
                title={MEGON_OPENAI_COMPATIBLE_PRESET.name}
                tag={language.t("dialog.provider.tag.recommended")}
                connectLabel={language.t("common.connect")}
                onConnect={() =>
                  dialog.show(() => (
                    <DialogOpenAICompatible
                      defaults={MEGON_OPENAI_COMPATIBLE_PRESET}
                      iconID="Megon"
                      apiKeyOnly
                    />
                  ))
