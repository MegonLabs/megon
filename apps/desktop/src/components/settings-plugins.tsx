...
import { useDialog } from "@megon/ui/context"
...
function SkillDetails(props: { url: string, item: SkillStoreViewItem }) {
  ...
  const [skillInfo] = createResource(props.url, async (url) => {
    ...
    const rawHtml = proseElements.map(el => el.innerHTML).join('<hr class="my-8 border-white/10" />') || "No description provided."
  ...
  const installSkill = async (item: SkillStoreViewItem) => {
    ...
    try {
      ...
      showToast({
        ...
        description: `${item.displayName} is now available in Megon.`,
        variant: "success",
        duration: 4000,
      })
    } catch (error) {
  ...
  const uninstallSkill = async (item: SkillStoreViewItem) => {
    ...
    try {
      ...
      showToast({
        ...
        description: `${item.displayName} was removed from Megon.`,
        variant: "success",
        duration: 4000,
      })
    } catch (error) {
  ...
  return (
    <div class="mx-auto w-full max-w-[900px] space-y-10 p-6 text-foreground lg:p-10">
      <div class="space-y-1">
        ...
        <p class="text-[15px] text-muted-foreground">Work with Megon across your favorite tools</p>
      </div>
