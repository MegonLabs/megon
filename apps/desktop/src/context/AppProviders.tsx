...
import { I18nProvider } from "@megon/ui/context"
import { DialogProvider } from "@megon/ui/context/dialog"
import { MarkedProvider } from "@megon/ui/context/marked"
...
function desktopStorage(storage?: string): AsyncStorage {
  return {
    getItem: async (key) => {
      const bridge = window.megon?.storage
...
async function resolveSidecarServerUrl() {
  if (!window.megon) return "http://localhost:4096"

  const current = window.megon.getServerUrl()
  if (current) return current

  const result = await nativeApi.invoke("MEGON_server_start")
