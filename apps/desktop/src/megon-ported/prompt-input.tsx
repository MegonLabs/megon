import { useFilteredList } from "@megon/ui/hooks"
import { useSpring } from "@megon/ui/motion-spring"
import {
  createEffect,
  createMemo,
  createSignal,
...
import { DockShellForm, DockTray } from "@megon/ui/dock-surface"
import { Icon } from "@megon/ui/icon"
import { ProviderIcon } from "@megon/ui/provider-icon"
import { Tooltip, TooltipKeybind } from "@megon/ui/tooltip"
import { IconButton } from "@megon/ui/icon-button"
import { Select } from "@megon/ui/select"
import { Spinner } from "@megon/ui/spinner"
...
import { ImagePreview } from "@megon/ui/image-preview"
...
export const PromptInput: Component<PromptInputProps> = (props) => {
  ...
  const [inlineMessage, setInlineMessage] = createSignal<{ text: string; variant: "info" | "error" | "success" } | null>(null)
  let inlineMessageTimer: ReturnType<typeof setTimeout> | undefined
  const showInlineMessage = (text: string, variant: "info" | "error" | "success" = "info", duration = 4000) => {
    clearTimeout(inlineMessageTimer)
    setInlineMessage({ text, variant })
    if (duration > 0) {
      inlineMessageTimer = setTimeout(() => setInlineMessage(null), duration)
    }
  }
  ...
  onCleanup(() => clearTimeout(inlineMessageTimer))
  ...
  const openBrowserTab = () => {
    window.dispatchEvent(new CustomEvent("megon-open-browser-tab"))
  }
  ...
  onMount(() => {
    ...
    window.addEventListener("megon-browser-element-selected", handleBrowserElementSelected)
    onCleanup(() => window.removeEventListener("megon-browser-element-selected", handleBrowserElementSelected))
  })
  ...
  const improvePrompt = async () => {
    ...
    if (reason) {
      showInlineMessage(reason, "error")
    ...
    showInlineMessage("Improving prompt...", "info", 0)
    closePopover()
    ...
    try {
      ...
      showInlineMessage("Prompt improved", "success")
    } catch (error) {
      if (requestID !== improveRequestID) return
      ...
      showInlineMessage(description, "error")
  ...
  return (
    ...
    >
      ...
      >
        ...
        />
        <Show when={inlineMessage()}>
          <div
            class="agent-terminal-prompt-message flex items-center gap-2 px-4 py-2 text-[13px] font-medium rounded-lg mx-4 mb-1"
            classList={{
              "bg-blue-500/10 text-blue-400 border border-blue-500/20": inlineMessage()!.variant === "info",
              "bg-red-500/10 text-red-400 border border-red-500/20": inlineMessage()!.variant === "error",
              "bg-green-500/10 text-green-400 border border-green-500/20": inlineMessage()!.variant === "success",
            }}
            role="status"
            aria-live="polite"
          >
            <Show when={inlineMessage()!.variant === "error"}>
              <svg class="w-4 h-4 flex-shrink-0" viewBox="0 0 16 16" fill="currentColor"><path fill-rule="evenodd" d="M8 15A7 7 0 1 0 8 1a7 7 0 0 0 0 14ZM8 4a.75.75 0 0 1 .75.75v3.5a.75.75 0 0 1-1.5 0v-3.5A.75.75 0 0 1 8 4Zm0 8a1 1 0 1 0 0-2 1 1 0 0 0 0 2Z" clip-rule="evenodd"/></svg>
            </Show>
            <Show when={inlineMessage()!.variant === "success"}>
              <svg class="w-4 h-4 flex-shrink-0" viewBox="0 0 16 16" fill="currentColor"><path fill-rule="evenodd" d="M8 15A7 7 0 1 0 8 1a7 7 0 0 0 0 14Zm3.78-9.77a.75.75 0 0 0-1.06-1.06L7 8.94 5.28 7.22a.75.75 0 0 0-1.06 1.06l2.25 2.25a.75.75 0 0 0 1.06 0l4.25-4.25Z" clip-rule="evenodd"/></svg>
            </Show>
            ...
            >
              <div
                ...
                onContextMenu={(e) => {
                  ...
                  menu.style.cssText = "position:fixed;z-index:9999;min-width:160px;background:var(--surface-raised-base);border:1px solid var(--border-weak-base);border-radius:8px;padding:4px;box-shadow:var(--shadow-lg);font-size:13px;font-family:var(--font-family-sans);"
                  ...
                  cutItem.style.cssText = "display:flex;align-items:center;gap:8px;width:100%;padding:6px 10px;border:none;background:none;cursor:pointer;border-radius:4px;text-align:left;color:var(--text-base);"
                  cutItem.onmouseenter = () => (cutItem.style.background = "var(--surface-raised-base-hover)")
                  cutItem.onmouseleave = () => (cutItem.style.background = "none")
                  cutItem.onclick = async () => {
                    if (hasSelection) {
                  ...
                  if (!hasSelection) cutItem.style.opacity = "0.4"
                  cutItem.style.pointerEvents = hasSelection ? "auto" : "none"

                  const copyItem = document.createElement("button")
                  copyItem.type = "button"
                  copyItem.textContent = "Copy"
                  copyItem.style.cssText = "display:flex;align-items:center;gap:8px;width:100%;padding:6px 10px;border:none;background:none;cursor:pointer;border-radius:4px;text-align:left;color:var(--text-base);"
                  copyItem.onmouseenter = () => (copyItem.style.background = "var(--surface-raised-base-hover)")
                  ...
                  if (!hasSelection) copyItem.style.opacity = "0.4"
                  copyItem.style.pointerEvents = hasSelection ? "auto" : "none"

                  const pasteItem = document.createElement("button")
                  pasteItem.type = "button"
                  pasteItem.textContent = "Paste"
                  pasteItem.style.cssText = "display:flex;align-items:center;gap:8px;width:100%;padding:6px 10px;border:none;background:none;cursor:pointer;border-radius:4px;text-align:left;color:var(--text-base);"
                  pasteItem.onmouseenter = () => (pasteItem.style.background = "var(--surface-raised-base-hover)")
                  pasteItem.onmouseleave = () => (pasteItem.style.background = "none")
                  ...
                  separator.style.cssText = "height:1px;background:var(--border-weak-base);margin:4px 0;"
