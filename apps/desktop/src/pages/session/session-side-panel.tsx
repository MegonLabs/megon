...
import { Tabs } from "@megon/ui/tabs"
import { ResizeHandle } from "@megon/ui/resize-handle"
import { IconButton } from "@megon/ui/icon-button"
import { Icon } from "@/components/ui/icon"
import { Tooltip } from "@megon/ui/tooltip"
import { useDialog } from "@megon/ui/context/dialog"
import type { SnapshotFileDiff, VcsFileDiff } from "@megon/sdk/v2"
  ...
  const openAddTabDialog = () => {
    dialog.show(() => (
      <DialogAddTab
        ...
        onOpenTerminal={() => {
          window.dispatchEvent(new CustomEvent("megon-open-terminal-tab", { detail: { cwd: getActiveDirectory() } }))
        }}
        onOpenBrowser={() => {
            ...
            <div class="size-full min-w-0 h-full min-h-0 bg-background-base flex flex-col">
              ...
              >
                <div class="sticky top-0 z-10 shrink-0 flex bg-background-base border-b border-border-weaker-base">
                  <Tabs.List>
                    <Tabs.Trigger value="all-files" onClick={() => selectTab("all-files")}>
                      <div class="flex items-center gap-1.5">
                        <Icon name="file-tree" size="small" class="size-3.5" />
                        <div>{language.t("session.tab.allFiles")}</div>
                      </div>
                    </Tabs.Trigger>
                ...
                >
                  <Show when={props.activeTabId() === "all-files"}>
                    {props.allFilesPanel()}
                  </Show>
                </Tabs.Content>
                <Tabs.Content
