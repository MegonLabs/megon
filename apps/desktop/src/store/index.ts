...
import { toLocalMegonSession } from '@/utils/megon-session';
...
interface AppActions {
  ...
  syncMegonSessions: (projectId: string, sessions: MegonSession[]) => Promise<void>;
...
const [store, setStore] = createStore<AppState>({
  ...
  availableShells: [],
  themeId: getStoredValue(STORAGE_KEYS.themeId) ?? 'Megon',
...
export const actions: AppActions = {
  ...
  syncMegonSessions: async (projectId: string, sessions: MegonSession[]) => {
    const project = store.projects.find((p) => p.id === projectId);
    if (!project) return;
    ...
    const normalized = sessions
      ...
      .filter((session) => !session.time?.archived)
        .map((session): Session => {
          return toLocalMegonSession(session, {
