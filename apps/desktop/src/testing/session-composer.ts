import type { Todo } from "@megon/sdk/v2"

export const composerEvent = "megon:e2e:composer"
...
export const composerEnabled = () => {
  ...
  return (window as ComposerWindow).__MEGON_e2e?.composer?.enabled === true
}

const root = () => {
  if (!composerEnabled()) return
  const state = (window as ComposerWindow).__MEGON_e2e?.composer
  if (!state) return
  state.sessions ??= {}
