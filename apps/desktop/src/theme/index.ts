...
export type MegonThemeVariant = ThemeVariant
export type MegonTheme = DesktopTheme

const files = import.meta.glob<{ default: MegonTheme }>("./themes/*.json", { eager: true })

export const MEGON_THEMES: Record<string, MegonTheme> = Object.fromEntries(
  Object.values(files).map((mod) => [mod.default.id, mod.default]),
)

export const MEGON_THEME_LIST = Object.values(MEGON_THEMES).sort((a, b) => a.name.localeCompare(b.name))

const FALLBACK_THEME_ID = 'Megon'
...
export const resolveAppThemeTokens = (theme: MegonTheme, mode: 'light' | 'dark'): Record<string, string> => {
...
export const applyAppTheme = (theme: MegonTheme, mode: ResolvedThemeMode): Record<string, string> => {
...
export const getThemeById = (id: string | null | undefined): MegonTheme => MEGON_THEMES[id ?? ''] ?? MEGON_THEMES[FALLBACK_THEME_ID]
