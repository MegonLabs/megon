export const getStoredValue = (key: string): string | null => {
  ...
  const nativeValue = window.megon?.storage.getItem(undefined, key);
  ...
  if (legacyValue !== null) {
    void window.megon?.storage.setItem(undefined, key, legacyValue).catch(() => undefined);
  }
...
export const setStoredValue = (key: string, value: string | null): void => {
  ...
  const storage = window.megon?.storage;
  if (!storage) return;
