...
const envPath = process.env.MEGON_BIN_PATH
if (envPath) run(envPath)

const scriptDir = path.dirname(fs.realpathSync(__filename))
const binary = process.platform === "win32" ? "megon.exe" : "megon"
const bundled = path.join(scriptDir, "..", "dist", "bin", binary)

if (!fs.existsSync(bundled)) {
  console.error("Megon CLI binary was not found. Run `bun run build` in apps/megon before publishing.")
  process.exit(1)
}
