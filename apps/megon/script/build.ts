...
const channel = process.env.MEGON_CHANNEL || "local"
...
const outFile = path.join(dir, "dist/bin", process.platform === "win32" ? "megon.exe" : "megon")
const compileTarget =
...
const result = await Bun.build({
  ...
  compile: {
    ...
    outfile: outFile,
    execArgv: [`--user-agent=megon/${version}`, "--use-system-ca", "--"],
    windows: {},
  },
  define: {
    MEGON_VERSION: JSON.stringify(version),
    MEGON_CHANNEL: JSON.stringify(channel),
    MEGON_MIGRATIONS: JSON.stringify(migrations),
    MEGON_WORKER_PATH: JSON.stringify(workerPath.replaceAll("\\", "/")),
    MEGON_LIBC: process.platform === "linux" ? JSON.stringify(process.env.MEGON_LIBC || "glibc") : "undefined",
