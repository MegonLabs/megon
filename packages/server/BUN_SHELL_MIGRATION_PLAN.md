...
Replace runtime Bun shell template-tag usage in `packages/megon/src` with a unified `Process` API in `util/process.ts`.
...
- Runtime Bun `$` usage in `packages/megon/src` is removed except:
