...
- Author package entrypoint is `@megon-ai/plugin/tui`.
...
```json
{
  "$schema": "https://megon.ai/tui.json",
  "theme": "smoke-theme",
  "plugin": ["@acme/megon-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  }
}
...
- Import types from `@megon-ai/plugin/tui`.
- `@megon-ai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@megon-ai/plugin/tui"
...
- The value is a semver range checked against the running megon version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.megon` is absent, no check is performed (backward compatible).
...
- `megon plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `megon plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.megon` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.megon`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.megon`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.megon` dir or `megon_CONFIG_DIR`, megon installs `@megon-ai/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@megon-ai/plugin/tui`.
...
- Local plugins persist installed themes under the local `.megon/themes` area near the plugin config source.
...
- `--pure` / `megon_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.megon/plugins/tui-smoke.tsx`
- Local vim plugin: `.megon/plugins/tui-vim.tsx`
- Local smoke config: `.megon/tui.json`
- Local smoke theme: `.megon/plugins/smoke-theme.json`
