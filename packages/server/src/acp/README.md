...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for megon.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal megon sessions
...
```bash
megon_ENABLE_QUESTION_TOOL=1 megon acp
```
...
- **Terminal Integration**: Full terminal support via megon's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | megon acp
```
...
ACP sessions map cleanly to megon's internal session model:
