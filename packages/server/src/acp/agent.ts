...
import type { AssistantMessage, Event, OpencodeClient, SessionMessageResponse, ToolPart } from "@megon/sdk/v2"
...
export namespace ACP {
  ...
  export class Agent implements ACPAgent {
    ...
    async initialize(params: InitializeRequest): Promise<InitializeResponse> {
      ...
      const authMethod: AuthMethod = {
        description: "Run `megon auth login` in the terminal",
        name: "Login with megon",
        id: "megon-login",
      }
    ...
    private async processMessage(message: SessionMessageResponse) {
      ...
      for (const part of message.parts) {
        if (part.type === "tool") {
          ...
          // megon stores files internally as { type: "file", url, filename, mime }.
  ...
  async function defaultModel(config: ACPConfig, cwd?: string): Promise<{ providerID: ProviderID; modelID: ModelID }> {
    ...
    const megonProvider = providers.find((p) => p.id === "megon")
    if (megonProvider) {
      if (megonProvider.models["big-pickle"]) {
        return { providerID: ProviderID.megon, modelID: ModelID.make("big-pickle") }
      }
      const [best] = Provider.sort(Object.values(megonProvider.models))
      if (best) {
        return {
