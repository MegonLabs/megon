...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run megon",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.code === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for megon session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found megon session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await Process.text(["megon", "import", sessionUrl], {
                  nothrow: true,
                })
        ...
        UI.println()
        UI.println("Starting megon...")
        UI.println()

        const megonArgs = sessionId ? ["-s", sessionId] : []
        const megonProcess = Process.spawn(["megon", ...megonArgs], {
          stdin: "inherit",
          stdout: "inherit",
          stderr: "inherit",
          cwd: process.cwd(),
        })
        const code = await megonProcess.exited
        if (code !== 0) throw new Error(`megon exited with code ${code}`)
      },
    })
  },
})
