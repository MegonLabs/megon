...
import { Binary } from "@megon/util/binary"
  ...
  init: () => {
    ...
    event.subscribe((event) => {
      switch (event.type) {
        ...
        case "session.diff":
          setStore("session_diff", event.properties.sessionID, event.properties.diff as { file: string; patch: string; additions: number; deletions: number; status?: "added" | "deleted" | "modified" }[])
          break
        ...
        async sync(sessionID: string) {
          ...
          setStore(
            produce((draft) => {
              ...
              draft.session_diff[sessionID] = (diff.data ?? []) as typeof draft.session_diff[string]
            }),
          )
