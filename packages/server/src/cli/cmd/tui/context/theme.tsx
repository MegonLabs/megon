...
import megon from "./theme/megon.json" with { type: "json" }
...
import type { TuiThemeCurrent } from "@megon/plugin/tui"
  ...
  init: (props: { mode: "dark" | "light" }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        const active = config.theme ?? (saved === "Megon" || typeof saved !== "string" ? "system" : saved)
        draft.active = active
        draft.ready = false
      }),
    )
    ...
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes.megon, store.mode)
    })
...
function generateSystem(colors: TerminalColors, mode: "dark" | "light"): ThemeJson {
  ...
  return {
    theme: {
      ...
      // Main background is transparent so megon blends into the user's terminal,
