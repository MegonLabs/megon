...
import type { GlobalEvent } from "@megon/sdk/v2"
import type { EventSource } from "./context/sdk"

declare global {
  const MEGON_WORKER_PATH: string
}
...
async function target() {
  if (typeof MEGON_WORKER_PATH !== "undefined") return MEGON_WORKER_PATH
...
export const TuiThreadCommand = cmd({
  command: "$0 [project]",
  describe: "start megon tui",
  builder: (yargs) =>
    withNetworkOptions(yargs)
      .positional("project", {
        type: "string",
        describe: "path to start megon in",
      })
      .option("model", {
        type: "string",
        alias: ["m"],
