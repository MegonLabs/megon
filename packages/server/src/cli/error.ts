...
export function FormatError(input: unknown) {
  if (MCP.Failed.isInstance(input))
    return `MCP server "${input.data.name}" failed. Note, megon does not support MCP authentication yet.`
  ...
  if (Provider.ModelNotFoundError.isInstance(input)) {
    ...
    return [
      ...
      `Try: \`megon models\` to list available models`,
      `Or check your config (megon.json) provider/model names`,
    ].join("\n")
  }
