...
import { NamedError } from "@megon/util/error"
...
export namespace Config {
  ...
  // Scoped token store for WellKnown auth — avoids leaking API keys to child processes via process.env
  const wellKnownTokens = new Map<string, string>()

  export function getWellKnownToken(envKey: string): string | undefined {
    return wellKnownTokens.get(envKey)
  }
  ...
  function systemManagedConfigDir(): string {
    switch (process.platform) {
      case "darwin":
        return "/Library/Application Support/megon"
      case "win32":
        return path.join(process.env.ProgramData || "C:\\ProgramData", "megon")
      default:
        return "/etc/megon"
    }
  }
  ...
  // Keys injected by macOS/MDM into the managed plist that are not megon config
  ...
  /**
   * Parse raw JSON (from plutil conversion of a managed plist) into megon config.
  ...
  export const McpOAuth = z
    .object({
      ...
      clientSecret: z
        ...
        .optional()
        .describe(
          "OAuth client secret (if required by the authorization server). NOTE: Stored as plaintext in config files. Use dynamic client registration or environment variables for production deployments.",
        ),
  ...
  export const Server = z
    .object({
      ...
      mdnsDomain: z.string().optional().describe("Custom domain name for mDNS service (default: megon.local)"),
  ...
  export const Info = z
    .object({
      ...
      server: Server.optional().describe("Server configuration for megon serve and web commands"),
      command: z
        .record(z.string(), Command)
        .optional()
        .describe("Command configuration, see https://megon.org/docs/commands"),
      ...
      agent: z
        ...
        .catchall(Agent)
        .optional()
        .describe("Agent configuration, see https://megon.org/docs/agents"),
      provider: z
  ...
  export class Service extends Context.Service<Service, Interface>()("@megon/Config") {}

  function globalConfigFile() {
    // Prefer an existing megon file, then config files; when none
    // exist yet, create megon.json so new global config is written under megon's identity.
    const candidates = ["megon.jsonc", "megon.json", "config.json"].map((file) =>
      path.join(Global.Path.config, file),
    )
    for (const file of candidates) {
      if (existsSync(file)) return file
    }
    return path.join(Global.Path.config, "megon.json")
  }
      ...
      Effect.gen(function* () {
        ...
        const loadConfig = Effect.fnUntraced(function* (
          ...
          const normalized = (() => {
            ...
            delete copy.tui
            log.warn("tui keys in megon config are deprecated; move them to tui.json", { path: source })
            return copy
          })()
          ...
          if (parsed.success) {
            if (!parsed.data.$schema && isFile) {
              parsed.data.$schema = "https://megon.org/config.json"
        ...
        const loadGlobal = Effect.fnUntraced(function* () {
          let result: Info = pipe(
            ...
            mergeDeep(yield* loadFile(path.join(Global.Path.config, "megon.json"))),
            mergeDeep(yield* loadFile(path.join(Global.Path.config, "megon.jsonc"))),
            // Megon config files.
            mergeDeep(yield* loadFile(path.join(Global.Path.config, "megon.json"))),
            mergeDeep(yield* loadFile(path.join(Global.Path.config, "megon.jsonc"))),
          )
            ...
            yield* Effect.promise(() =>
              import(pathToFileURL(legacy).href, { with: { type: "toml" } })
                .then(async (mod) => {
                  ...
                  result["$schema"] = "https://megon.org/config.json"
        ...
        const loadInstanceState = Effect.fnUntraced(function* (ctx: InstanceContext) {
          ...
          const scope = Effect.fnUntraced(function* (source: string) {
            ...
            if (source === "MEGON_CONFIG_CONTENT") return "local"
          ...
          for (const [key, value] of Object.entries(auth)) {
            if (value.type === "wellknown") {
              ...
              wellKnownTokens.set(value.key, value.token)
              process.env[value.key] = value.token
              log.debug("fetching remote config", { url: `${url}/.well-known/megon` })
              const headers: Record<string, string> = {}
              if (value.token) headers["Authorization"] = `Bearer ${value.token}`
              const response = yield* Effect.promise(() =>
                fetch(`${url}/.well-known/megon`, { headers }),
              )
              if (!response.ok) {
              ...
              if (!remoteConfig.$schema) remoteConfig.$schema = "https://megon.org/config.json"
              const source = `${url}/.well-known/megon`
          ...
          if (Flag.MEGON_CONFIG) {
            yield* merge(Flag.MEGON_CONFIG, yield* loadFile(Flag.MEGON_CONFIG))
            log.debug("loaded custom config", { path: Flag.MEGON_CONFIG })
          }

          if (!Flag.MEGON_DISABLE_PROJECT_CONFIG) {
            for (const file of yield* Effect.promise(() =>
              ConfigPaths.projectFiles("megon", ctx.directory, ctx.worktree),
            )) {
          ...
          if (Flag.MEGON_CONFIG_DIR) {
            log.debug("loading config from MEGON_CONFIG_DIR", { path: Flag.MEGON_CONFIG_DIR })
          }

          const deps: Promise<void>[] = []

          for (const dir of unique(directories)) {
            if (ConfigPaths.PROJECT_DIRS.some((d) => dir.endsWith(d)) || dir === Flag.MEGON_CONFIG_DIR) {
          ...
          if (process.env.MEGON_CONFIG_CONTENT) {
            const source = "MEGON_CONFIG_CONTENT"
            const next = yield* loadConfig(process.env.MEGON_CONFIG_CONTENT, {
              dir: ctx.directory,
              source,
            })
            yield* merge(source, next, "local")
            log.debug("loaded custom config from MEGON_CONFIG_CONTENT")
          }
          ...
          if (activeOrg) {
            yield* Effect.gen(function* () {
              ...
              if (Option.isSome(tokenOpt)) {
                Env.set("MEGON_CONSOLE_TOKEN", tokenOpt.value)
              }
          ...
          if (Flag.MEGON_PERMISSION) {
            result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.MEGON_PERMISSION))
          }
          ...
          if (Flag.MEGON_DISABLE_AUTOCOMPACT) {
            result.compaction = { ...result.compaction, auto: false }
          }
          if (Flag.MEGON_DISABLE_PRUNE) {
