...
const TUI_SCHEMA_URL = "https://megon.org/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from megon.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const megon = await MegonFiles(input)
  for (const file of megon) {
...
async function MegonFiles(input: { directories: string[]; managed: string }) {
  const project = Flag.MEGON_DISABLE_PROJECT_CONFIG
    ? []
    : await ConfigPaths.projectFiles("megon", Instance.directory, Instance.worktree)
  const files = [...project, ...ConfigPaths.fileInDirectory(Global.Path.config, "megon")]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "megon"))
  }
  if (Flag.MEGON_CONFIG) files.push(Flag.MEGON_CONFIG)
  files.push(...ConfigPaths.fileInDirectory(input.managed, "megon"))
