...
export namespace TuiConfig {
  ...
  const state = Instance.state(async () => {
    let projectFiles = Flag.MEGON_DISABLE_PROJECT_CONFIG
      ? []
    ...
    projectFiles = Flag.MEGON_DISABLE_PROJECT_CONFIG
      ? []
      : await ConfigPaths.projectFiles("tui", Instance.directory, Instance.worktree)
    ...
    for (const dir of unique(directories)) {
      if (!ConfigPaths.PROJECT_DIRS.some((d) => dir.endsWith(d)) && dir !== Flag.MEGON_CONFIG_DIR) continue
  ...
  async function load(text: string, configFilepath: string): Promise<Info> {
    ...
    // (mirroring the old megon.json shape) still get their settings applied.
