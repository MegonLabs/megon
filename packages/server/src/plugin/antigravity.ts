import type { Hooks, PluginInput } from "@megon/plugin"
...
function ensureAntigravityCredentials() {
  const missing: string[] = []
  if (!CLIENT_ID) missing.push("ANTIGRAVITY_CLIENT_ID")
  if (!CLIENT_SECRET) missing.push("ANTIGRAVITY_CLIENT_SECRET")
  if (missing.length > 0) {
    throw new Error(
      `Antigravity plugin requires environment variables: ${missing.join(", ")}. ` +
        `Set them in your .env file or shell environment. ` +
        `See https://megon.org/docs/config#antigravity for setup instructions.`,
    )
  }
}
