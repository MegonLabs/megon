import type { Hooks, PluginInput, Plugin as PluginInstance, PluginModule } from "@megon/plugin"
import { Config } from "../config/config"
import { Bus } from "../bus"
import { Log } from "../util/log"
import { createOpencodeClient } from "@megon/sdk"
...
export namespace Plugin {
  ...
  export class Service extends Context.Service<Service, Interface>()("@megon/Plugin") { }
      ...
      const state = yield* InstanceState.make<State>(
        Effect.fn("Plugin.state")(function* (ctx) {
          ...
          const client = createOpencodeClient({
            ...
            headers: Flag.MEGON_SERVER_PASSWORD
              ? {
                Authorization: `Basic ${Buffer.from(`${Flag.MEGON_SERVER_USERNAME ?? "megon"}:${Flag.MEGON_SERVER_PASSWORD}`).toString("base64")}`,
              }
              : undefined,
          ...
          const plugins = Flag.MEGON_PURE ? [] : (cfg.plugin_origins ?? [])
          if (Flag.MEGON_PURE && cfg.plugin_origins?.length) {
