...
export const DEPRECATED_PLUGIN_PACKAGES = ["megon-openai-codex-auth", "megon-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, megonVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(megonVersion) || semver.major(megonVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.megon
  if (typeof range !== "string") return
  if (!semver.satisfies(megonVersion, range)) {
    throw new Error(`Plugin requires megon ${range} but running ${megonVersion}`)
  }
}
