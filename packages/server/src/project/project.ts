...
export namespace Project {
  ...
  export class Service extends Context.Service<Service, Interface>()("@megon/Project") {}
  ...
  > = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const fakeVcs = Info.shape.vcs.parse(Flag.MEGON_FAKE_VCS)
      ...
      const readCachedProjectId = Effect.fnUntraced(function* (dir: string) {
        return yield* fs.readFileString(pathSvc.join(dir, "megon")).pipe(
          Effect.map((x) => x.trim()),
      ...
      const fromDirectory = Effect.fn("Project.fromDirectory")(function* (directory: string) {
        ...
        const data: DiscoveryResult = yield* Effect.gen(function* () {
          ...
          if (!id) {
            ...
            if (id) {
              yield* fs.writeFileString(pathSvc.join(worktree, ".git", "megon"), id).pipe(Effect.ignore)
            }
          }
        ...
        if (Flag.MEGON_EXPERIMENTAL_ICON_DISCOVERY)
