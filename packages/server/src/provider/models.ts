...
export namespace ModelsDev {
  ...
  function url() {
    return Flag.MEGON_MODELS_URL || "https://models.dev"
  }
  ...
  export const Data = lazy(async () => {
    const result = await Filesystem.readJson(Flag.MEGON_MODELS_PATH ?? filepath).catch(() => {})
    ...
    if (Flag.MEGON_DISABLE_MODELS_FETCH) return {}
...
if (!Flag.MEGON_DISABLE_MODELS_FETCH && !process.argv.includes("--get-yargs-completions")) {
  ModelsDev.refresh()
  setInterval(
    async () => {
