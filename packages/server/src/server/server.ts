  ...
  export function ControlPlaneRoutes(upgrade: UpgradeWebSocket, app = new Hono(), opts?: { cors?: string[] }): Hono {
    return app
      ...
      .use((c, next) => {
        ...
        const password = Flag.MEGON_SERVER_PASSWORD
        if (!password) return next()
        const username = Flag.MEGON_SERVER_USERNAME ?? "megon"
      ...
      .use(
        cors({
          ...
          origin(input) {
            ...
            if (/^https:\/\/([a-z0-9-]+\.)*megon\.ai$/.test(input)) return input
