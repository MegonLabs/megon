...
declare const MEGON_MIGRATIONS: { sql: string; timestamp: number; name: string }[] | undefined
...
export namespace Database {
  const DB_BASENAME = "megon"
  const LEGACY_DB_BASENAME = "megon"

  function channelDbName(base: string) {
    if (["latest", "beta", "prod"].includes(CHANNEL) || Flag.MEGON_DISABLE_CHANNEL_DB) return `${base}.db`
  ...
  // Earlier builds named the database after "megon". Rename any legacy file
  // (and its -wal/-shm sidecars) to the megon identity in-place so existing
  ...
  export const Path = iife(() => {
    if (Flag.MEGON_DB) {
      if (Flag.MEGON_DB === ":memory:" || path.isAbsolute(Flag.MEGON_DB)) return Flag.MEGON_DB
      return path.join(Global.Path.data, Flag.MEGON_DB)
    }
  ...
  export const Client = lazy(() => {
    ...
    const entries =
      typeof MEGON_MIGRATIONS !== "undefined"
        ? MEGON_MIGRATIONS
        : migrations(path.join(import.meta.dirname, "../../migration"))
    if (entries.length > 0) {
      log.info("applying migrations", {
        count: entries.length,
        mode: typeof MEGON_MIGRATIONS !== "undefined" ? "bundled" : "dev",
      })
      if (Flag.MEGON_SKIP_MIGRATIONS) {
