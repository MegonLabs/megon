...
export const BrowserTool = Tool.define(
  ...
  Effect.succeed({
    description: [
      "Control megon's embedded browser tab. Use this when a task needs a real browser session.",
    ...
    execute: (params: z.infer<typeof Parameters>, ctx: Tool.Context) =>
      Effect.gen(function* () {
        const baseUrl = process.env.MEGON_BROWSER_CONTROL_URL
        const token = process.env.MEGON_BROWSER_CONTROL_TOKEN
        if (!baseUrl || !token) {
          const metadata: BrowserMetadata = { action: params.action, available: false }
          return {
            title: "Browser unavailable",
            output: "The embedded browser bridge is not running. Start megon in the Electron app to use this tool.",
            metadata,
          }
        }
