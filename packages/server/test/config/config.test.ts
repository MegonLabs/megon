...
async function writeManagedSettings(settings: object, filename = "megon.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Filesystem.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "megon.json") {
...
async function check(map: (dir: string) => string) {
  ...
  try {
    await writeConfig(globalTmp.path, {
      $schema: "https://megon.ai/config.json",
      snapshot: false,
    })
...
test("ignores legacy tui keys in megon config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await writeConfig(dir, {
        $schema: "https://megon.ai/config.json",
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Filesystem.readText(path.join(tmp.path, "megon.json"))
...
test("resolves env templates in account config with account token", async () => {
  const originalControlToken = process.env["MEGON_CONSOLE_TOKEN"]
  ...
  const fakeAccount = Layer.mock(Account.Service)({
    ...
    config: () =>
      Effect.succeed(
        Option.some({
          provider: { megon: { options: { apiKey: "{env:MEGON_CONSOLE_TOKEN}" } } },
        }),
      ),
  ...
  try {
    ...
    if (originalControlToken !== undefined) {
      process.env["MEGON_CONSOLE_TOKEN"] = originalControlToken
    } else {
      delete process.env["MEGON_CONSOLE_TOKEN"]
    }
  }
})
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "megon.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .megon directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const megonDir = path.join(dir, ".megon")
      await fs.mkdir(megonDir, { recursive: true })
      const agentDir = path.join(megonDir, "agent")
...
test("loads agents from .megon/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const megonDir = path.join(dir, ".megon")
      await fs.mkdir(megonDir, { recursive: true })

      const agentsDir = path.join(megonDir, "agents")
...
test("loads commands from .megon/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const megonDir = path.join(dir, ".megon")
      await fs.mkdir(megonDir, { recursive: true })

      const commandDir = path.join(megonDir, "command")
...
test("loads commands from .megon/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const megonDir = path.join(dir, ".megon")
      await fs.mkdir(megonDir, { recursive: true })

      const commandsDir = path.join(megonDir, "commands")
...
test("does not try to install dependencies in read-only MEGON_CONFIG_DIR", async () => {
  if (process.platform === "win32") return
  ...
  const prev = process.env.MEGON_CONFIG_DIR
  process.env.MEGON_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.MEGON_CONFIG_DIR
    else process.env.MEGON_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable MEGON_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
  ...
  const install = spyOn(Npm, "install").mockImplementation(async (dir: string) => {
    const mod = path.join(dir, "node_modules", "@megon-ai", "plugin")
    await fs.mkdir(mod, { recursive: true })
    await Filesystem.write(
      path.join(mod, "package.json"),
      JSON.stringify({ name: "@megon/plugin", version: "1.0.0" }),
    )
  })
...
test("dedupes concurrent config dependency installs for the same dir", async () => {
  ...
  const run = spyOn(Npm, "install").mockImplementation(async (d: string) => {
    ...
    const mod = path.join(d, "node_modules", "@megon-ai", "plugin")
    await fs.mkdir(mod, { recursive: true })
    await Filesystem.write(
      path.join(mod, "package.json"),
      JSON.stringify({ name: "@megon/plugin", version: "1.0.0" }),
...
test("serializes config dependency installs across dirs", async () => {
  ...
  const run = spyOn(Npm, "install").mockImplementation(async (dir: string) => {
    ...
    const mod = path.join(cwd, "node_modules", "@megon-ai", "plugin")
    await fs.mkdir(mod, { recursive: true })
    await Filesystem.write(
      path.join(mod, "package.json"),
      JSON.stringify({ name: "@megon/plugin", version: "1.0.0" }),
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Filesystem.write(
        path.join(dir, "megon.json"),
        JSON.stringify({ $schema: "https://megon.ai/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .megon config
      const projectDir = path.join(dir, "project")
      const megonDir = path.join(projectDir, ".megon")
      await fs.mkdir(megonDir, { recursive: true })
      ...
      // Local .megon config with different plugins
      await Filesystem.write(
        path.join(megonDir, "megon.json"),
        JSON.stringify({
          $schema: "https://megon.ai/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .megon config with some overlapping plugins
      await Filesystem.write(
        path.join(megonDir, "megon.json"),
        JSON.stringify({
          $schema: "https://megon.ai/config.json",
...
test("keeps plugin origins aligned with merged plugin list", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const local = path.join(project, ".megon")
...
// Note: preload.ts sets MEGON_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("local .megon config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .megon directory config enables it
      const megonDir = path.join(dir, ".megon")
      await fs.mkdir(megonDir, { recursive: true })
      await Filesystem.write(
        path.join(megonDir, "megon.json"),
...
test("project config overrides remote well-known config", async () => {
  ...
  globalThis.fetch = mock((url: string | URL | Request) => {
    ...
    if (urlStr.includes(".well-known/megon")) {
      fetchedUrl = urlStr
    ...
    await provideTmpdirInstance(
      () =>
        Config.Service.use((svc) =>
          Effect.gen(function* () {
            const config = yield* svc.get()
            expect(fetchedUrl).toBe("https://example.com/.well-known/megon")
...
describe("resolvePluginSpec", () => {
  test("keeps package specs unchanged", async () => {
    await using tmp = await tmpdir()
    const file = path.join(tmp.path, "megon.json")
    expect(await Config.resolvePluginSpec("oh-my-megon@2.4.3", file)).toBe("oh-my-megon@2.4.3")
...
describe("deduplicatePluginOrigins", () => {
  ...
  test("keeps path plugins separate from package plugins", () => {
    const plugins = ["oh-my-megon@2.4.3", "file:///project/.megon/plugin/oh-my-megon.js"]
  ...
  test("deduplicates direct path plugins by exact spec", () => {
    const plugins = ["file:///project/.megon/plugin/demo.ts", "file:///project/.megon/plugin/demo.ts"]

    const result = dedupe(plugins)

    expect(result).toEqual(["file:///project/.megon/plugin/demo.ts"])
  })
  ...
  test("loads auto-discovered local plugins as file urls", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const megonDir = path.join(projectDir, ".megon")
        const pluginDir = path.join(megonDir, "plugin")
        await fs.mkdir(pluginDir, { recursive: true })
...
describe("MEGON_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["MEGON_DISABLE_PROJECT_CONFIG"]
    process.env["MEGON_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["MEGON_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["MEGON_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .megon/ directories when flag is set", async () => {
    const originalEnv = process.env["MEGON_DISABLE_PROJECT_CONFIG"]
    process.env["MEGON_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .megon directory with a command
          const megonDir = path.join(dir, ".megon", "command")
          await fs.mkdir(megonDir, { recursive: true })
          await Filesystem.write(path.join(megonDir, "test-cmd.md"), "# Test Command\nThis is a test command.")
        },
      })
      await Instance.provide({
        directory: tmp.path,
        fn: async () => {
          ...
          // Project .megon should NOT be in directories list
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["MEGON_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["MEGON_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["MEGON_CONFIG_DIR"]
      process.env["MEGON_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["MEGON_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["MEGON_CONFIG_DIR"]
      } else {
        process.env["MEGON_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("MEGON_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["MEGON_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["MEGON_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["MEGON_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["MEGON_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await Config.get()
          // Should load from MEGON_CONFIG_DIR, not project
...
describe("MEGON_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in MEGON_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["MEGON_CONFIG_CONTENT"]
    const originalTestVar = process.env["TEST_CONFIG_VAR"]
    process.env["TEST_CONFIG_VAR"] = "test_api_key_12345"
    process.env["MEGON_CONFIG_CONTENT"] = JSON.stringify({
      $schema: "https://megon.ai/config.json",
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["MEGON_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["MEGON_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in MEGON_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["MEGON_CONFIG_CONTENT"]
...
test("parseManagedPlist strips MDM metadata keys", async () => {
  const config = await Config.parseManagedPlist(
    JSON.stringify({
      PayloadDisplayName: "megon Managed",
      PayloadIdentifier: "ai.megon.managed.test",
      PayloadType: "ai.megon.managed",
...
test("parseManagedPlist handles empty config", async () => {
  const config = await Config.parseManagedPlist(
    JSON.stringify({ $schema: "https://megon.ai/config.json" }),
    "test:mobileconfig",
  )
  expect(config.$schema).toBe("https://megon.ai/config.json")
})
