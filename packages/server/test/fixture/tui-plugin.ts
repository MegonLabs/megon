import { createOpencodeClient } from "@megon/sdk/v2"
...
type Opts = {
  ...
  count?: Count
  keybind?: Partial<Record<string, unknown>>
...
export function createTuiPluginApi(opts: Opts = {}): HostPluginApi {
  ...
  let selected = opts.theme?.selected ?? "Megon"
  const key = {
  ...
  return {
    ...
    attention: { notify: async () => ({}), configure: async () => ({}), status: () => ({}) } as any,
    get client() {
      return client()
    },
    event: {
    ...
    keymap: {} as any,
    mode: {} as any,
    tuiConfig: opts.tuiConfig ?? {} as any,
    kv: {
      get: kvGet,
      set(name, value) {
        kv[name] = value
