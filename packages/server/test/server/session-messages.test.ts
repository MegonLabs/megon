...
async function withoutWatcher<T>(fn: () => Promise<T>) {
  ...
  const prev = process.env.MEGON_EXPERIMENTAL_DISABLE_FILEWATCHER
  process.env.MEGON_EXPERIMENTAL_DISABLE_FILEWATCHER = "true"
  try {
    return await fn()
  } finally {
    if (prev === undefined) delete process.env.MEGON_EXPERIMENTAL_DISABLE_FILEWATCHER
    else process.env.MEGON_EXPERIMENTAL_DISABLE_FILEWATCHER = prev
  }
}
