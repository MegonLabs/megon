...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "megon.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "megon.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "megon.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "megon.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["megon.json", "megon.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })

      expect(result).toEqual([
        path.join(tmp.path, "megon.json"),
        path.join(tmp.path, "megon.jsonc"),
        path.join(project, "megon.json"),
        path.join(project, "megon.jsonc"),
      ])
    })
  })
