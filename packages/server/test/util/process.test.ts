...
describe("util.process", () => {
  ...
  test("merges environment overrides", async () => {
    const out = await Process.run(node('process.stdout.write(process.env.MEGON_TEST ?? "")'), {
      env: {
        MEGON_TEST: "set",
      },
    })
  ...
  test("uses shell in run on Windows", async () => {
    ...
    const out = await Process.run(["set", "MEGON_TEST_SHELL"], {
      shell: true,
      env: {
        MEGON_TEST_SHELL: "ok",
      },
    ...
    expect(out.code).toBe(0)
    expect(out.stdout.toString()).toContain("MEGON_TEST_SHELL=ok")
  })
