...
import { getDirectory as _getDirectory, getFilename } from "@megon/util/path"
...
export function UserMessageDisplay(props: { message: UserMessage; parts: PartType[]; actions?: UserActions }) {
  ...
  return (
    <ContextMenu>
      <ContextMenu.Trigger as="div" data-component="user-message" ref={messageRef}>
        ...
        <Show when={text()}>
          <>
            ...
            <div data-slot="user-message-copy-wrapper">
              ...
              <Show when={props.actions?.undoChanges}>
                <Tooltip value="Undo Changes" placement="top" gutter={4}>
                  <IconButton
                    icon="undo"
                    size="normal"
                    variant="ghost"
              ...
              <Show when={props.actions?.regenerate}>
                <Tooltip value="Regenerate" placement="top" gutter={4}>
                  <IconButton
                    icon="refresh"
                    size="normal"
                    variant="ghost"
      ...
      <ContextMenu.Portal>
        <ContextMenu.Content>
          <ContextMenu.Item onSelect={handleCopy}>
            <ContextMenu.ItemLabel>{i18n.t("ui.message.copy")}</ContextMenu.ItemLabel>
          </ContextMenu.Item>
          <ContextMenu.Item onSelect={handleSelectAll}>
            <ContextMenu.ItemLabel>{i18n.t("ui.message.selectAll")}</ContextMenu.ItemLabel>
          </ContextMenu.Item>
        </ContextMenu.Content>
      </ContextMenu.Portal>
    </ContextMenu>
...
PART_MAPPING["text"] = function TextPartDisplay(props) {
  ...
  let partRef: HTMLDivElement | undefined

  const handleSelectAll = () => {
    if (!partRef) return
    const range = document.createRange()
    range.selectNodeContents(partRef)
    const selection = window.getSelection()
    selection?.removeAllRanges()
    selection?.addRange(range)
  }
  ...
  return (
    <Show when={text()}>
      <ContextMenu>
        <ContextMenu.Trigger as="div" data-component="text-part" ref={partRef}>
