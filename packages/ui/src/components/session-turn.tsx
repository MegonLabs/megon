import {
  ...
  Part as PartType,
} from "@megon/sdk/v2/client"
import type { SessionStatus } from "@megon/sdk/v2"
import { useData } from "../context"
import { useFileComponent } from "../context/file"

import { Binary } from "@megon/util/binary"
import { getDirectory, getFilename } from "@megon/util/path"
  ...
  const diffs = createMemo(() => {
    ...
    return files
      .reduceRight<SnapshotFileDiff[]>((result, diff) => {
        if (!diff.file || seen.has(diff.file)) return result
        seen.add(diff.file)
        result.push(diff)
        return result
      }, [])
