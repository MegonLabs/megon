...
export function ensureFindShortcuts() {
  installShortcuts()
}
...
export function unregisterFindHost(host: FindHost) {
  hosts.delete(host)
  if (current === host) {
    current = undefined
    clearHighlightFind()
...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("megon-find")
  api.delete("megon-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("megon-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("megon-find", new Highlight(...rest))
    return true
  }
...
export type ChatFindOptions = {
  container: () => HTMLElement | undefined
}

export function createChatFind(opts: ChatFindOptions) {
  let input: HTMLInputElement | undefined
  let hits: Range[] = []
  ...
  const scanChat = (container: HTMLElement, value: string): Range[] => {
    const needle = value.toLowerCase()
    const ranges: Range[] = []
    ...
    let node = walker.nextNode()
    while (node) {
      if (!(node instanceof Text)) { node = walker.nextNode(); continue }
      const text = node.data
      const hay = text.toLowerCase()
      let at = hay.indexOf(needle)
      if (at === -1) { node = walker.nextNode(); continue }
  ...
  const clearHighlights = () => {
    const api = (globalThis as unknown as { CSS?: { highlights?: any }; Highlight?: any }).CSS?.highlights
    if (api) {
      api.delete("megon-chat-find")
      api.delete("megon-chat-find-current")
    }
    hits = []
  }
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    const api = (globalThis as unknown as { CSS?: { highlights?: any }; Highlight?: any }).CSS?.highlights
    const Highlight = (globalThis as unknown as { Highlight?: any }).Highlight
    if (!api || typeof Highlight !== "function") return

    api.delete("megon-chat-find")
    api.delete("megon-chat-find-current")

    const active = ranges[currentIndex]
    if (active) api.set("megon-chat-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("megon-chat-find", new Highlight(...rest))
  }
  ...
  const apply = (args?: { reset?: boolean; scroll?: boolean }) => {
    if (!open()) return
    const value = query().trim()
    if (!value) { clearHighlights(); setState("count", 0); return }

    const container = opts.container()
    if (!container) return

    const ranges = scanChat(container, value)
    const total = ranges.length
    const desired = args?.reset ? 0 : index()
    const currentIndex = total ? Math.min(desired, total - 1) : 0
    ...
    setHighlights(ranges, currentIndex)
    if (args?.scroll && ranges[currentIndex]) scrollToRange(ranges[currentIndex])
  }
  ...
  const next = (dir: 1 | -1) => {
    ...
    setState("index", next)
    if (hits[next]) {
      scrollToRange(hits[next])
      setHighlights(hits, next)
    }
  }
  ...
  onMount(() => {
    registerFindHost(host)
    onCleanup(() => {
      unregisterFindHost(host)
      clearHighlights()
    })
  })
  ...
  createEffect(() => {
    ...
    if (!container) return
    const onScroll = () => {
      if (hits.length > 0) setHighlights(hits, index())
    }
  ...
  return {
    ...
    next,
    refresh: (args?: { reset?: boolean; scroll?: boolean }) => apply(args),
    onPointerDown: () => { target = host; opts.container()?.focus({ preventScroll: true }) },
    onFocus: () => { target = host },
    onInputKeyDown: (event: KeyboardEvent) => {
