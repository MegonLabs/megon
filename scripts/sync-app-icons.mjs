...
const devSourceIcon = path.join(appDir, "src", "assets", "icon", "megon-dev.png");
...
const devIconPng = path.join(devIconDir, "megon-dev.png");
const devIconIco = path.join(devIconDir, "megon-dev.ico");
...
async function syncDevTaskbarIcon() {
  ...
  try {
    ...
    console.warn("[icons] dev ico skipped:", e instanceof Error ? e.message : e);
  }
}
...
async function main() {
  ...
  await syncPng();
  try { await syncIco(); } catch (e) { console.warn("[icons] ico skipped:", e instanceof Error ? e.message : e); }
  await syncIcns();
