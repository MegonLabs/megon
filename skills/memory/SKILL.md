...
name: memory
description: >
  Persistent, token-efficient project memory. When ON, maintains a `.megon/memory/` folder of
...
`.megon/memory/` that capture everything needed to resume the project cold — with zero prior
...
> running a command — your FIRST action is to read `.megon/memory/`.** Memory is the
...
2. **Never forget.** You do not rely on chat history or your own recollection. The files in
   `.megon/memory/` are your only trusted memory. If it is not written there, you treat it as
   not remembered.
...
If `.megon/memory/` does not exist yet, your memory-first action is to BOOTSTRAP it (see below)
...
brain in `.megon/memory/` must be enough to fully reconstruct context.

## The Loop (run every response)

1. **LOAD (start of response) — progressive disclosure, not a full dump.**
   - If `.megon/memory/` does NOT exist → first activation → go to BOOTSTRAP.
...
When `.megon/memory/` does not exist yet:

1. Create the folder `.megon/memory/`.
...
All files live in `.megon/memory/`. Each holds ONE concern. Keep them tight and current —
...
- `.megon/memory/` is local project state. Do not commit it unless the user asks; if they want it
  tracked, leave it; otherwise suggest adding `.megon/` to `.gitignore`.
